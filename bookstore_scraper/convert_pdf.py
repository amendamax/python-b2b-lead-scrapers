...
def convert_excel_to_pdf(xlsx_path, pdf_path):
    """
    Converts the Steel Blue styled books Excel sheet into a crisp landscape PDF.
    Ensures all columns (including the price comparison chart) fit neatly on a 
    single page width, letting rows flow naturally over pages to keep text clear.
    """
    ...
    try:
        ...
        ws.PageSetup.FitToPagesWide = 1 # Force all columns and charts to fit on 1 page wide
...
if __name__ == "__main__":
    xlsx_file = "scraped_books_report.xlsx"
    pdf_file = "scraped_books_report.pdf"
