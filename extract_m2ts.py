...
# Căutăm doar fișierele .m2ts omise anterior
EXTENSIONS = {'.m2ts'}

def extract_and_sort_m2ts():
    total_moved = 0
    for src in SOURCE_FOLDERS:
        ...
        print(f"Incep scanarea in: {src} pentru fisiere .m2ts")
            ...
            for file in files:
                ...
                if ext in EXTENSIONS:
                    ...
                    try:
                        ...
                        total_moved += 1
                        print(f"Mutat: {file} -> {year_folder}")
                    except Exception as e:
                        print(f"Eroare la procesarea fisierului {source_path}: {e}")

    print(f"\nFinalizat! Au fost sortate si mutate pe ani {total_moved} fisiere .m2ts in {DEST_FOLDER}.")

if __name__ == "__main__":
    extract_and_sort_m2ts()
