...
# Search black (schwarz) and blue (blau) Macan Diesel 2016-2018
for col in ['schwarz', 'blau']:
    for page in range(1, 10):
        url = f'https://www.autoscout24.de/lst/porsche/macan/ve_diesel?sort=standard&desc=0&ustate=N%2CU&fregfrom=2016&fregto=2018&powerfrom=188&bodycolors={col}&page={page}'
        try:
            ...
            print(f"Error {col} page {page}: {e}")
            break

print(f"Total collected (Black/Blue): {len(all_listings)}")
...
for u, it in unique_urls.items():
    try:
        ...
        black_blue_matches.append(item_obj)
        print(f"[{country}] {city} | {cost} | {km} km | {reg} | {color} | Score:{score}/5 (360:{has_360}, Blind:{has_blind_spot}, Pano:{has_tetto}, Lane:{has_lane}, Priv:{has_privacy})")
    except Exception as e:
        print(f"Error {u}: {e}")

black_blue_matches.sort(key=lambda x: (x['score'], -float(str(x['price']).replace('€','').replace('.','').replace(',','.').strip() or 0)), reverse=True)

with open(r'C:\Users\bratu\Documents\antigravity\amazing-borg\black_blue_perfect_matches.json', 'w', encoding='utf-8') as f:
    json.dump(black_blue_matches, f, indent=2, ensure_ascii=False)
print("Saved black/blue matches!")
