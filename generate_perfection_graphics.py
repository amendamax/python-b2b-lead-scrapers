...
def generate_multiline_png(lines, filename, font_path, font_size=150, text_color=(57, 255, 20), padding=40, line_spacing=20):
    ...
    # Left-align for code blocks, but we can indent properly
    current_y = padding
    for i, line in enumerate(lines):
        w, h, offset_x, offset_y = line_sizes[i]
        # Keep left-aligned (with padding) instead of centering, since it's code with indentation
        x_pos = padding
...
if __name__ == "__main__":
    ...
    perfection_lines = [
        'if status != "perfect":',
        '    optimize_to_infinity()',
        'else:',
        '    # Just to be 100% sure',
        '    run_one_more_final_test()'
    ]
    ...
    for color_name, color_rgba in colors.items():
        generate_multiline_png(
            perfection_lines,
            os.path.join(output_dir, f"perfection_loop_{color_name}.png"),
